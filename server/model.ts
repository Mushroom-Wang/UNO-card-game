 ...
 */
export function areCompatible(card: Card, lastCardPlayed: Card | undefined) {
  if (typeof lastCardPlayed === 'undefined') {
    return false
  }
  return card.rank === lastCardPlayed.rank || card.suit === lastCardPlayed.suit || card.rank === "A" || lastCardPlayed.rank === "A"
}
...
/**
 * Step3: find a list of the players who have only 1 card left in their hand or fewer
 */
export function haveOneOrLessCard(GameState: GameState) {
  const playerCardsNumber = computePlayerCardCounts(GameState)
  const haveOneOrLessCardList: number[] = []
  for (let i: number = 0; i < playerCardsNumber.length; i++) {
    if (playerCardsNumber[i] <= 1) {
      haveOneOrLessCardList.push(i)
    }
  }
  return haveOneOrLessCardList
}
